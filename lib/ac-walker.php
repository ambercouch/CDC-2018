  ...
  if ($args->theme_location == 'services') {
    ...
    foreach ($sorted_menu_objects as $menu_object) {
      ...
      if (in_array($menu_object->object, array(' post', 'page', 'any_post_type'))) {
        ...
        $menu_object->title = has_post_thumbnail($menu_object->object_id) ? '<div class = "menu--services__wrapper" data-posbot="-62" data-posleft="0">' . get_the_post_thumbnail($menu_object->object_id, 'service-thumb') . '<div class="menu--services__title"><span class="menu--services__title-text">' . $menu_object->title . '</span></div></div> ' : $menu_object->title;
      }
    }
  }
